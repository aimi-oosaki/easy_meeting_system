...
<div class="form">
    ...
    <label for="${AttributeConst.EMP_TEAM.getValue()}">チーム</label><br />
    <select name="${AttributeConst.EMP_TEAM.getValue()}">
      <option>チームを選んでください</option>
      <c:forEach var="team" items="${teams}">
        <option value="${team.id}" <c:if test="${team.id == employee.team.id}">selected</c:if>>${team.name}</option>
      </c:forEach>
