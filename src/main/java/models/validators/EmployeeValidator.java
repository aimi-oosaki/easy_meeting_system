...
public class EmployeeValidator {
    ...
    public static List<String> validate(
        EmployeeService service, EmployeeView ev, Boolean codeDuplicateCheckFlag, Boolean passwordCheckFlag, Integer company_id) {
        List<String> errors = new ArrayList<String>();

        //会社CDのチェック
        String companyError = validateCompany(ev.getCompany_id());
        if (!companyError.equals("")) {
            errors.add(companyError);
        }

        //社員番号のチェック
        String codeError = validateCode(service, ev.getCode(), codeDuplicateCheckFlag, company_id);
        if (!codeError.equals("")) {
    ...
    private static String validateCode(EmployeeService service, String code, Boolean codeDuplicateCheckFlag, Integer company_id) {
        ...
        if (codeDuplicateCheckFlag) {
            ...
            long employeesCount = isDuplicateEmployee(service, code, company_id);
    ...
    private static long isDuplicateEmployee(EmployeeService service, String code, Integer company_id) {
        ...
        long employeesCount = service.countByCode(code, company_id);
